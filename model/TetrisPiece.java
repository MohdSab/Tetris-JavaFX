...
//        your code here!
        ...
        ArrayList<Integer> lowestY = new ArrayList<>();
        ArrayList<Integer> lowestX = new ArrayList<>();

        for (TetrisPoint point: points) {
            if (!lowestX.contains(point.x)) {
                lowestX.add(point.x);
            }
            if (!lowestY.contains(point.y)) {
                lowestY.add(point.y);
            }
        }

        this.height = lowestY.size();
        this.width = lowestX.size();

        int[] temp = new int[this.width];

        for (int x: lowestX) {
            smallestY = Double.POSITIVE_INFINITY;
            smallestY = helperIterator(points, x, smallestY);
            temp[x - Collections.min(lowestY)] = smallestY.intValue();
        }
        this.lowestYVals = temp;
    }

    public Double helperIterator(TetrisPoint[] points, int x, Double smallestY) {
        for (TetrisPoint point: points) {
            if (x == point.x && smallestY > point.y){
                smallestY = (double) point.y;
            }
        }
        return smallestY;
    }
    ...
    public boolean equals(Object obj) {
        ...
        List<TetrisPoint> pt1 = Arrays.asList(body);
        TetrisPiece piece = (TetrisPiece) obj;
        List<TetrisPoint> pt2 = Arrays.asList(piece.body);

        if (this == obj){
            return true;
        } else if (piece.body.length != body.length) {
            return false;
        } else if (!(obj instanceof model.TetrisPiece)){
            return false;
        }
        return pt1.containsAll(pt2);
    }
...
//        throw new UnsupportedOperationException(); //replace this!
        ...
        TetrisPiece curr = new TetrisPiece(root.body);
        return helperFastRotation(curr, curr);
    }
    private static TetrisPiece helperFastRotation(TetrisPiece curr, TetrisPiece piece){
        int x = 1;
        while (x > 0){
            TetrisPiece next = curr.computeNextRotation();
            if (piece.equals(next)){
                curr.next = piece;
                return piece;
            }
            curr.next = next;
...
//        throw new UnsupportedOperationException(); //replace this!
        ...
        TetrisPoint[] piece = new TetrisPoint[body.length];

        for (int i = 0 ; i < body.length ; i++) {
            piece[i] = new TetrisPoint(height - body[i].y - 1, body[i].x);
        }
        return (new TetrisPiece(piece));
    }
