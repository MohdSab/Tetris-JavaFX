...
public class TetrisBoard implements Serializable{
    private final int width; //board height and width
    ...
    private final boolean[][] backupGrid; //to back up your grid
    private final int[] backupColCounts; //to back up your row counts
    private final int[] backupRowCounts; //to back up your column counts
...
//        throw new UnsupportedOperationException(); //change this!
        ...
        int i = 0;
        while (i < height) {
            if (getColumnHeight(x + i) - temp[i] > total) {
                total = getColumnHeight(x + i) - temp[i];
            }
            i++;
        }
        return total;
...
//        throw new UnsupportedOperationException(); //replace this!
        ...
        for (TetrisPoint tetrisPoint : body) {
            XPiece = x + tetrisPoint.x;
            YPiece = y + tetrisPoint.y;

            if (checkBad(XPiece, YPiece, result) == ADD_BAD || checkBad(XPiece, YPiece, result) == ADD_OUT_BOUNDS){
                result = checkBad(XPiece, YPiece, result);
                break;
            }

            tetrisGrid[XPiece][YPiece] = true;

            if (getColumnHeight(XPiece) < YPiece + 1) {
                colCounts[XPiece] = YPiece + 1;
            }
            rowCounts[YPiece]++;

            if (rowCounts[YPiece] == width) {
    ...
    public int checkBad(int XPiece, int YPiece, int result) {
        if (XPiece < 0 || YPiece < 0 || XPiece >= width || YPiece >= height) {
            return ADD_OUT_BOUNDS;
        }

        if (tetrisGrid[XPiece][YPiece]) {
            return ADD_BAD;
        }
        return result;
    }
...
//        throw new UnsupportedOperationException(); //replace this!
        ...
        rowTo = 0; rowFrom = 1;
        while (rowFrom < getMaxHeight()) {
            if (!rowFilled && rowCounts[rowTo] == width) {
                rowFilled = true;
                rowsCleared++;
            }

            if (rowFilled && rowFrom < getMaxHeight() && rowCounts[rowFrom] == width) {
                do {
                    rowsCleared++;
                    rowFrom++;
                } while (rowFilled && rowFrom < getMaxHeight() && rowCounts[rowFrom] == width);
            }
        ...
        while (i < colCounts.length) {
            colCounts[i] -= rowsCleared;
            if (colCounts[i] > 0 && !tetrisGrid[i][colCounts[i] - 1]) {
                colCounts[i] = 0;
    ...
    private void copySingleRow(int rowTo, int rowFrom) { //DELETE

        if(rowFrom<getMaxHeight())
        {
            for(int i = 0;i<width;i++)
            {
                tetrisGrid[i][rowTo] = tetrisGrid[i][rowFrom];
    ...
    private void fillEmptyRows(int lowRow, int highRow) { //DELETE
